...
class VPCStack(cdk.Stack):   
    def __init__(self, scope: Construct, id: str,  **kwargs) -> None:
        ...
        for i, subnet in enumerate(app_prd_vpc.private_subnets):
            cdk.Tags.of(subnet).add('Name', f"app-prd-vpc-private-subnet-{i+1}", exclude_resource_types=['AWS::EC2::RouteTable'])
            cdk.Tags.of(subnet).add('Name', f"app-prd-vpc-private-route-table-{i+1}", include_resource_types=['AWS::EC2::RouteTable'])
        ...
        for i, subnet in enumerate(management_prd_vpc.private_subnets):
            cdk.Tags.of(subnet).add('Name', f"management_prd_vpc-private-subnet-{i+1}", exclude_resource_types=['AWS::EC2::RouteTable'])
            cdk.Tags.of(subnet).add('Name', f"management_prd_vpc-private-route-table-{i+1}", include_resource_types=['AWS::EC2::RouteTable'])
